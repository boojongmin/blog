...
스프링 웹플럭스는 5.0 번전 이후에 추가되었습니다. 완전한 논블럭킹, 
http://www.reactive-streams.org/[Reactive Streams] 지원, 역압(back pressure) 그리고 
Netty, Undertow, Servlet 3.1 이상 컨테이너와 같은 서버에서 동작합니다.
...
스프링 프레임워크에서 나란히 공존합니다. 각각 모듈은 옵션널합니다.
어플리케이션은 하나 또는 다른 모듈을 또는 둘다 사용할수 있습니다.
...
만약 퍼블리셔가 속도를 늦출수 없다면 (누가?) 버퍼링, 삭제 또는 실패를 결정해야합니다.
====
